...
try:
    ...
    class FairChemModel(torch.nn.Module, ModelInterface):
        ...
        checkpoint. It supports various model architectures and configurations supported
        by FairChem.
            ...
            *,  # force remaining arguments to be keyword-only
            ...
            if config_yml is not None:
                if isinstance(config_yml, str):
                    config, dupe_warnings, dupe_errors = load_config(config_yml)
                    if len(dupe_warnings) > 0:
                        print(
                            "Overwritten config parameters from included configs "
                            f"(non-included parameters take precedence): {dupe_warnings}"
                        )
                    if len(dupe_errors) > 0:
                        raise ValueError(
                            "Conflicting (duplicate) parameters in simultaneously "
                            f"included configs: {dupe_errors}"
                        )
                else:
                    config = config_yml
            ...
            if "backbone" in config["model"]:
                ...
                if dtype is not None:
                    try:
                        ...
                        print(
                            "WARNING: dtype not found in backbone, using default "
                            "model dtype"
                        )
            else:
            ...
            if seed is None:
                print(
                    "No seed has been set in model checkpoint or OCPCalculator! "
                    "Results may not be reproducible on re-run"
                )
            else:
        ...
        def forward(self, state: SimState | StateDict) -> dict:
            ...
            Args:
                state (SimState | StateDict): State object containing positions, cells,
                    atomic numbers, and other system information. If a dictionary is
                    provided, it will be converted to a SimState.
            ...
            for i, (n, c) in enumerate(
                ...
                data_list.append(
                    Data(
                        ...
                        natoms=n,
                        pbc=torch.tensor(
                            [state.pbc, state.pbc, state.pbc], dtype=torch.bool
                        ),
                    )
                )
